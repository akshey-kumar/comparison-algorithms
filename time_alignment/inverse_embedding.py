...
def fit_inverse_embedder(x0_tr, y0_tr, x0_tst, y0_tst):
...
if __name__ == '__main__':
    ...
    # points to inverse embed
    y_inv_embed = np.array([
        [-0.368, 1.104, -1.196],
        [-0.441,0.406,0.009],
        [-0.191, 0.020, 0.673],
        [-1.067, -0.309, 0.673]
    ])
    x_pred = inverse_embedder(y_inv_embed)
    for i, x_i in enumerate(x_pred):
        plt.figure(figsize=(5,2.5))
        for n, x_n in enumerate(x_i):
            plt.ylim(0,1.7)
            plt.scatter(n, x_n)
        plt.title(y_inv_embed[i])
